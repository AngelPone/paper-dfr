library(dplyr)
source('qp.R')

# Domain Setting
incoherent_domain = expand.grid(0:1, 0:1, 0:2)
coherent_domain = expand.grid(0:1, 0:1)
coherent_domain$Var3 = coherent_domain$Var1 + coherent_domain$Var2

r = nrow(coherent_domain)
q = nrow(incoherent_domain)

distance = matrix(0, nrow = r, ncol = q)
for (j in 1:r){
  for (k in 1:q){
    distance[j, k] = sum(abs(coherent_domain[j,] - incoherent_domain[k,]))
  }
}

history_length = 300
window_length = 60
test_length = 30

n = history_length+window_length+test_length

library(foreach)
cl <- parallel::makeCluster(4)
doParallel::registerDoParallel(cl)

loop_FUN <- function(i){
  f1 <- glarma_base_forecast(foo[i:(i+history_length-1), 1])
  f2 <- glarma_base_forecast(foo[i:(i+history_length-1), 2])
  f3 <- glarma_base_forecast(apply(foo[i:(i+history_length-1),], 1, sum), total = TRUE)
  list(pred=apply(expand.grid(f1, f2, f3), 1, prod),
       real=(foo[i+history_length,]),
       f1=f1,
       f2=f2,
       f3=f3)
}

loop_result <- function(res, l = window_length){
  pred <- sapply(res, function(x){x$pred})
  bu <- sapply(res, function(x){x$bu})
  real <- sapply(res, function(x){x$real}) %>% t()
  # dummy real observations matrix
  real_mat <- matrix(0, l, 4)
  for (i in 1:l){
    real_mat[i, which((coherent_domain$Var1 == real[i,1]) & 
                     (coherent_domain$Var2 == real[i,2]))] <- 1
  }
  list(pred=pred, real=t(real_mat), bu=bu)
}



# brier_score of reconciled forecast



scale_loop <- function(a){
  foo <- simulate_series(param_a = a, 
                         n = n, 
                         intercept = 0,
                         innovation_mu = c(0, 0),
                         innovation_Sigma = matrix(c(0.1, 0.05, 0.05, 0.2), 2, 2))
  glarma_base_forecast = glarma_base_forecast
  loop_FUN <- function(i){
    history_length = 300
    window_length = 60
    test_length = 30
    f1 <- glarma_base_forecast(foo[i:(i+history_length-1), 1])
    f2 <- glarma_base_forecast(foo[i:(i+history_length-1), 2])
    f3 <- glarma_base_forecast(apply(foo[i:(i+history_length-1),], 1, sum), total = TRUE)
    list(pred=apply(expand.grid(f1, f2, f3), 1, prod),
         bu=apply(expand.grid(f1, f2), 1, prod),
         real=(foo[i+history_length,]),
         f1=f1,
         f2=f2,
         f3=f3)
  }
  train_res <- foreach(i=1:(window_length)) %dopar% 
    loop_FUN(i)
  
  train_res <- loop_result(train_res)
  Ahat1 <- opt_fun(train_res$pred, train_res$real, distance, lambda = 0.01)
  Ahat05 <- opt_fun(train_res$pred, train_res$real, distance, lambda = 0.005)
  Ahat01 <- opt_fun(train_res$pred, train_res$real, distance, lambda = 0.001)
  Ahat0 <- opt_fun(train_res$pred, train_res$real, distance, lambda = 0)
  
  test_base <- foreach(i=(window_length+1):(window_length+test_length)) %dopar% 
    loop_FUN(i)
  test_base <- loop_result(test_base, l=test_length)
  
  Ahat0[Ahat0<1e-5] = 0
  Ahat1[Ahat1<1e-5] = 0
  list(bs1=brier_score(Ahat1 %*% test$pred, test_base$real),
       bs0=brier_score(Ahat0 %*% test$pred, test_base$real),
       bs05=brier_score(Ahat05 %*% test$pred, test_base$real),
       bs01=brier_score(Ahat01 %*% test$pred, test_base$real),
       bsbase=brier_score(test_base$pred[c(1, 6, 7, 12),], test_base$real),
       bsbu=brier_score(test_base$bu, test_base$real),
       Ahat0=Ahat0,
       Ahat1=Ahat1,
       Ahat01=Ahat01, Ahat05=Ahat05)
}
foo = scale_loop(a = runif(1, 0.5, 0.7))

scale_up_res <- list()

for (i in 1:100){
  print(i)
  a = runif(1, 0.5, 0.7)
  scale_up_res[[i]] <- scale_loop(a)
  scale_up_res[[i]]$a <- a
}

res <- scale_up_res %>% sapply(function(x){c(x$bs1, x$bs05, x$bs01, x$bs0, x$bsbase, x$bsbu)}) %>%
  t() %>% data.frame()

colnames(res) <- c("bs1", "bs05", "bs01", "bs0", "bsbase", "bsbu")

